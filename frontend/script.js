...
async function carregarMarcosLista() {
    try {
        ...
        console.log(`✅ ${marcos.length} marcos carregados para a lista`);
        ...
        container.innerHTML = marcos.map(marco => `
            ...
            ">
                <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: var(--space-2);">
                    ...
                    <span style="
                        background: ${marco.levantado ? 'var(--success-bg)' : 'var(--warning-bg)'};
                        color: ${marco.levantado ? 'var(--success)' : 'var(--warning)'};
                        padding: var(--space-1) var(--space-2);
                        border-radius: var(--radius);
                        font-size: var(--text-xs);
                        font-weight: 500;
                    ">${marco.levantado ? 'Levantado' : 'Pendente'}</span>
                </div>
                <div style="color: var(--text-secondary); font-size: var(--text-sm);">
                    <div>${marco.localizacao || 'Localização não informada'}</div>
                    <div style="margin-top: var(--space-1);">
                        ${marco.municipio || 'N/A'} - ${marco.uf || 'N/A'}
                    </div>
                </div>
            </div>
        `).join('');
