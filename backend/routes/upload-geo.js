 ...
 * Suporta: KML, KMZ, SHP (Shapefile), GeoJSON, DXF, DWG (via GDAL)
 */
...
const cadConverter = require('../converters/cad-converter');
...
const upload = multer({
    ...
    fileFilter: (req, file, cb) => {
        const allowedExtensions = ['.kml', '.kmz', '.shp', '.zip', '.geojson', '.json', '.dxf', '.dwg'];
...
/**
 * Verifica se as coordenadas são geográficas válidas (lat/lng)
 */
function isValidGeographicCoordinate(coords) {
    // Verifica se todos os pontos estão dentro dos limites válidos
    for (const ring of (Array.isArray(coords[0][0]) ? coords : [coords])) {
...
async function processarDXF(filePath) {
    ...
    for (const entity of dxf.entities) {
        if (entity.type === 'LWPOLYLINE' || entity.type === 'POLYLINE') {
            // Remover dimensão Z - usar apenas X e Y
            const coordinates = entity.vertices.map(v => [v.x, v.y]);

            // Verificar se tem pelo menos 3 pontos para formar um polígono
            if (coordinates.length < 3) {
                console.warn(`⚠️  Entidade ${entity.handle} ignorada: menos de 3 vértices`);
                continue;
            }
    ...
    if (features.length === 0) {
        throw new Error('Nenhuma entidade válida (POLYLINE/LWPOLYLINE) encontrada no DXF');
    }
...
async function calcularMedidas(geojson) {
    // Verificar se as coordenadas são geográficas válidas
    const coords = geojson.coordinates;
    const isGeoCoords = isValidGeographicCoordinate(coords);

    let query;
    if (isGeoCoords) {
        // Coordenadas geográficas - transformar de 4326 para 31982
        query = `
            SELECT
        ...
        // Coordenadas projetadas (sistema local) - assumir que já estão em metros
        // Usar SRID 0 (não definido) e depois atribuir 31982 para calcular
        query = `
            SELECT
                ST_Area(ST_SetSRID(ST_GeomFromGeoJSON($1), 31982)) as area_m2,
                ST_Perimeter(ST_SetSRID(ST_GeomFromGeoJSON($1), 31982)) as perimetro_m
        `;
        console.log('⚠️  Coordenadas em sistema local detectadas - assumindo unidade em metros (EPSG:31982)');
    }
...
async function inserirPropriedade(dados) {
    // Verificar se as coordenadas são geográficas
    const geojsonObj = JSON.parse(dados.geojson);
    const isGeoCoords = isValidGeographicCoordinate(geojsonObj.coordinates);

    let geometrySQL;
    if (isGeoCoords) {
        // Coordenadas geográficas - transformar de 4326 para 31982
        geometrySQL = 'ST_Transform(ST_GeomFromGeoJSON($10), 31982)';
    } else {
        // Coordenadas projetadas - assumir já estão em 31982
        geometrySQL = 'ST_SetSRID(ST_GeomFromGeoJSON($10), 31982)';
    }
    ...
    try {
        ...
        switch (ext) {
            ...
            case '.dxf':
                ...
                break;
            case '.dwg':
                // Usar GDAL para DWG (conversor avançado)
                console.log('   Usando GDAL para processar DWG...');
                geojson = await cadConverter.convertCADToGeoJSON(filePath);
                // Filtrar features relevantes (polígonos)
                geojson = cadConverter.filtrarFeaturesRelevantes(geojson);
                // Unir polígonos se necessário
                geojson = cadConverter.unirPoligonos(geojson);
                break;
            case '.geojson':
            case '.json':
